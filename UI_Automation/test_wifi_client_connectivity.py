...
def test_fronthaul_wifi_client_connectivity(request, ssh):
    ...
    for device in ["controller", "agent"]:
        ...
        print_step(f"Step 4: Process the WiFi scan results to extract valid BSSIDs")
        client_scan_lines = []
        ...
        print_step(f"Step 5: Select the AP BSSID that is currently visible")
        ...
        print_step(f"Step 6: Connect the client to BSSID :{target_bssid} via nmcli")
        ...
        print_step("Step 7: Validate the client IP address assignment and internet connectivity on the wireless interface")
        # Verify Client IP assignment and internet connectivity
        try:
            utils.verify_client_ip_and_internet(request, ssh, 7)
            print_success("Client IP assignment and internet connectivity verified.")
