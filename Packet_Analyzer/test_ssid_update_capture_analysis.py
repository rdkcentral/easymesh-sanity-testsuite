...
def test_capture_and_analyze_packets_with_ssid_update(page, paths, request, ssh):
    ...
    results = verify_ssid_update_in_controller_and_agent(page, request, ssh, new_ssid, 2, paths)
    ...
    if results[0] != new_ssid or results[1] != new_ssid:
        ...
        check.is_true(False, f"\nSSID mismatch between controller/agent devices. Expected SSID: {new_ssid}, Actual SSID on controller: {results[0]}, Actual SSID on agent: {results[1]}")
    else:
